...
import { tilesData } from "@/components/ManpowerPage/tilesData";
...
export function meta() {
  return [
    { title: "Manpower & Staffing Solutions | Emdee Techno Services" },
    { name: "description", content: "Professional manpower services, IT staffing, resource augmentation, and skilled workforce solutions for your organization." },
    { property: "og:title", content: "Manpower & Staffing Solutions | Emdee Techno Services" },
    { property: "og:description", content: "Professional manpower services, IT staffing, resource augmentation, and skilled workforce solutions." },
    { property: "og:url", content: "https://emdee.in/manpower" }
  ];
}
...
export default function ManpowerPage() {
  ...
  const isMobileIntroInView = useInView(mobileIntroRef, {
    once: true,
    amount: 0.2, // Trigger earlier for better UX
  });

  const desktopIntroRef = useRef(null);
  const isDesktopIntroInView = useInView(desktopIntroRef, {
    once: false,
    amount: 0.01, // Trigger when even 1% is in viewport
  });
  ...
  // Hide scroll icon when user scrolls down
  useEffect(() => {
    const handleScroll = () => {
      if (window.scrollY > 10 && !hasScrolled) {
        setHasScrolled(true);
      }
      // Hide scroll icon when scrolled past 100px
  ...
  // Exit animation for intro text when tile 1 reaches top 40%
  useEffect(() => {
    const handleIntroExit = () => {
      if (!tileRefs.current[0]?.current) return;

      // Get the image element inside tile 1
      ...
      const triggerPoint = viewportHeight * 0.55; // 55% from top

      if (imageRect.top <= triggerPoint && imageRect.top >= 0) {
        // Calculate progress: 0 when just reaching triggerPoint, 1 when fully past
    ...
    handleIntroExit(); // Check on mount
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] font-qanelas-soft">
      ...
      <div className="hidden md:block pt-20">
        {/* Hero section wrapper for absolute positioning */}
        <div className="relative">
          ...
          >
            <img
              ref={desktopImageRef}
              src="https://ucarecdn.com/5d71182f-52bd-45c8-9b35-70d5d650c83e/-/format/auto/"
              alt="IT Manpower"
        ...
        >
          ...
          >
            Across most of its technology deployments, EDPL not only supplies IT
            infrastructure or software but also provides trained manpower
            embedded at client premises to operationalise, manage and sustain
            these systems. This model extends across e-governance platforms,
            utility operations and digital service delivery, where people,
            processes and technology must work together at scale. In addition to
            this integrated approach, EDPL has also executed explicit IT and BPO
            manpower deployments (over 25,000 cumulatively), supplying dedicated
            personnel for large, structured government projects.
          </motion.p>
        </div>

        {/* Two-column layout: Index (35%) + Tiles (65%) */}
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8 pb-32">
          {/* Navigation Indicator - now position: fixed, no wrapper needed */}
          <ITESNavigationIndicator
            ...
            customIndexTitles={{
              1: "Bangla Sahayata Kendra",
              5: "IT personnel (Govt.)",
              7: "Auxillary manpower",
            }}
          />

          {/* Tiles - full width since indicator is fixed */}
      ...
      <div className="md:hidden pt-20">
        ...
        >
          <img
            src="https://ucarecdn.com/e742179c-8953-4bd2-a014-8cd55cacc066/-/format/auto/"
            alt="IT Manpower"
            className="w-full h-auto object-contain"
            ref={mobileImageRef}
            onLoad={() => setHeroImageLoaded(true)}
...
const MobileTileCard = ({
  ...
  useEffect(() => {
    const handleAnimation = () => {
      ...
      // Exit animation: starts when tile's bottom reaches 85% from top
      const exitTriggerStart = viewportHeight * 0.85;
      const exitTriggerEnd = -50; // Slightly above viewport top
  ...
  // Calculate animation style based on progress
  const getAnimationStyle = () => {
    ...
    if (entry < 1) {
      // Entry animation: fade in and move up from below
